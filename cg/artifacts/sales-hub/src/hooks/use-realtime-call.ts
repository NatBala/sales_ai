...
export function useRealtimeCall(options: UseRealtimeCallOptions) {
  ...
  const itemAudioStartTimeRef = useRef<number | null>(null);
  ...
  const handleRealtimeMessage = useCallback((event: MessageEvent) => {
    ...
    switch (type) {
      ...
      case "input_audio_buffer.speech_started": {
        accumulatedTranscriptRef.current = "";
        lastAgentPreviewRef.current = "";
        // Only cancel if the agent is actively speaking — avoids "no active responses" error
        if (agentSpeakingRef.current) {
          updateAgentSpeaking(false);
          sendRealtimeEvent({ type: "response.cancel" });
          // Only truncate if a meaningful amount of audio was played (>300ms)
          // to avoid the model restarting its script from the top
          const interruptedItemId = currentItemIdRef.current;
          const audioEndMs = itemAudioStartTimeRef.current
            ? Math.max(0, Date.now() - itemAudioStartTimeRef.current)
            : 0;
          if (interruptedItemId && audioEndMs > 300) {
            sendRealtimeEvent({
          ...
          itemAudioStartTimeRef.current = null;
        }
        break;
      }
        ...
        try {
          ...
          if (name === "book_meeting") {
            const booking: BookingProposal = {
              date: parsed.date ?? "",
              time: parsed.time ?? "",
              agendaTopic: parsed.agendaTopic ?? "Capital Group working session",
      ...
      case "response.output_item.added": {
        ...
        if (item?.id) {
          ...
          itemAudioStartTimeRef.current = Date.now();
        }
        break;
      }
  ...
  const startCall = useCallback(async (
    ...
    const initialResponse = startOptions.initialResponse === undefined
      ? {
          ...
          instructions:
            'The advisor just picked up the phone. In your first sentence, introduce yourself as "Maya, the Capital Group Scheduling AI assistant," then ask if they have 30 seconds. Keep it to 1-2 sentences.',
        }
    ...
    const connectAttempt = async (iceWaitMs: number) => {
      ...
      const sessionRes = await fetch(resolveApiUrl(sessionPath), {
        method: "POST",
        headers: {
      ...
      if (!sessionRes.ok) {
        let detail = "";
        try {
          const errorPayload = JSON.parse(sessionResText) as { error?: string; details?: string };
          detail = errorPayload.details || errorPayload.error || "";
        } catch {
          detail = sessionResText;
        }
        throw new Error(detail ? `Session creation failed: ${sessionRes.status} - ${detail}` : `Session creation failed: ${sessionRes.status}`);
      }

      const session = JSON.parse(sessionResText) as RealtimeSessionResponse;
