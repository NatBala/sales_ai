...
PORTFOLIO DIAGNOSTICS: Portfolio Solutions team has 30+ members, 15 CFA charterholders, 2,500+ consulting engagements per year. Meeting angle: diagnostic review, not replacement pitch.

TAX-AWARE / DIRECT INDEXING: Daily tax-loss harvesting scans. Potential 1%–2% or more in after-tax alpha for certain taxable clients. Use cases: concentrated positions, recurring gains, ESG preferences. Meeting angle: when direct indexing beats a simple ETF.

FIXED INCOME: Fixed Income Group manages $2.7 trillion. 85% of active fixed income funds outperformed peers over 10 years. New fixed income model portfolios launched in 2025. Municipal bonds for taxable HNW clients.
...
function detectBooking(text: string): BookingDetection {
  ...
  const agendaTopic = topicMatch?.[1]?.trim() || "Vanguard working session";
