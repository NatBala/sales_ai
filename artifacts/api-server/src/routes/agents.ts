...
const LEAD_ME_TRANSCRIPTION_PROMPT = `Transcribe a spoken lead-search query for Vanguard's advisor dataset.
...
const LEAD_ME_TRANSCRIPT_NORMALIZATION_PROMPT = `You are correcting a spoken lead-search query for Vanguard's advisor dataset.
...
const FOLLOW_ME_TRANSCRIPT_NORMALIZATION_PROMPT = `You are transcribing audio about Vanguard investing products and services.
Return only the corrected transcript.

Normalization rules:
1. Prefer the official Vanguard spelling for products, tools, accounts, services, share classes, and benchmarks when the spoken audio plausibly matches a known Vanguard term.
2. Preserve uppercase tickers and acronyms exactly: examples include VTI, VOO, VTSAX, VMFXX, ETF, IRA, RMD, NAV, APY, FDIC, CFP, CRSP, and FTSE.
3. Preserve numerals and punctuation inside official names when they are part of the product name or plan type: examples include 529, 403(b), SEP-IRA, 401(k), 0-3 Month Treasury Bill ETF, and Target Retirement 2035 Fund.
4. Normalize common spoken variants to the official term when context is clearly about Vanguard investing. Examples: 'V G' or 'VG' -> 'Vanguard'; 'five twenty nine' -> '529'; 'four oh three b' -> '403(b)'; 'sep ira' -> 'SEP-IRA'; 'U G M A / U T M A' -> 'UGMA/UTMA'; 'footsie' -> 'FTSE'; 'crisp' -> 'CRSP'; 'prime cap' -> 'PRIMECAP'.
5. Do not rewrite official product names into generic English phrases. Example: keep 'LifeStrategy' as one word; keep 'Admiral Shares' and 'Investor Shares' exactly; prefer 'Wellesley' over phonetic misspellings.
...
7. If a common English word conflicts with a known Vanguard term, prefer the Vanguard term only when the surrounding context is clearly investing, advice, retirement, taxes, accounts, or portfolio management.
8. In meeting-recap context, if the speaker says 'VG' while referring to products, materials, outlook, funds, comparisons, webinars, or follow-up actions, normalize it to 'Vanguard', not the letters 'VG'.

Vanguard glossary:
Advice services:
Vanguard Digital Advisor; Vanguard Personal Advisor; Vanguard Personal Advisor Select; Vanguard Personal Advisor Wealth Management; Tax-Efficient Retirement Strategy; Trust Services; CFP

Accounts and plans:
Vanguard Brokerage Account; Cash Plus Account; Roth IRA; Traditional IRA; SEP-IRA; Individual 401(k); 403(b) Services; UGMA/UTMA; 529 account; The Vanguard 529 Plan; Vanguard 529 portfolios; Target Enrollment Portfolios

Tools:
Investor Questionnaire; IRA contribution calculator; Retirement income calculator; Inherited RMD Calculator; Portfolio Watch; Portfolio Tester; Quick start tool; Watchlist; Compare ETFs and mutual funds; 529 tax benefits by state

Benchmarks and finance terms:
CRSP US Total Market Index; S&P 500 Index; FTSE Global All Cap Index; FTSE Global All Cap ex US Index; Bloomberg US Aggregate Float Adjusted Bond Index; FTSE 3-Month U.S. Treasury Bill Index; Admiral Shares; Investor Shares; Institutional Shares; ETF; NAV; SEC yield; RMD; APY; FDIC; REIT

High-priority funds and ETFs:
VOO; Vanguard S&P 500 ETF; VFIAX; Vanguard 500 Index Fund Admiral Shares; VTI; Vanguard Total Stock Market ETF; VTSAX; Vanguard Total Stock Market Index Fund Admiral Shares; BND; Vanguard Total Bond Market ETF; VBTLX; Vanguard Total Bond Market Index Fund Admiral Shares; VXUS; Vanguard Total International Stock ETF; VTIAX; Vanguard Total International Stock Index Fund Admiral Shares; VT; Vanguard Total World Stock ETF; VMFXX; Vanguard Federal Money Market Fund; VTINX; Vanguard Target Retirement Income Fund; VTTHX; Vanguard Target Retirement 2035 Fund; VSVNX; Vanguard Target Retirement 2070 Fund; VASIX; Vanguard LifeStrategy Income Fund; VSMGX; Vanguard LifeStrategy Moderate Growth Fund; VWELX; Vanguard Wellington Fund Investor Shares; VWINX; Vanguard Wellesley Income Fund Investor Shares; VPMCX; Vanguard PRIMECAP Fund Investor Shares; VNQ; Vanguard Real Estate ETF; VUG; Vanguard Growth ETF; VTV; Vanguard Value ETF; VIG; Vanguard Dividend Appreciation ETF

If the audio contains partial names, prefer the closest official Vanguard match from the glossary above.
  ...
  try {
    const resp = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: `You parse natural language queries about Vanguard advisor leads into structured filters and ranking intent.
    ...
    try {
      const scoreResp = await openai.chat.completions.create({
        ...
        messages: [
          {
            role: "system",
            content: `You are a sales intelligence AI for Vanguard financial services salespeople. Match advisors to the query, explain the fit, and keep reasoning grounded in the provided advisor data. Segments: A=top tier, B=high value, C=mid-market, D=developing, E=emerging. XC=exclusive channel, FC=flexible channel. Return valid JSON only.`,
          },
          {
            role: "user",
            content: `Query: "${query}"
...
async function generateCoachScenario(input: {
  ...
  const response = await openai.chat.completions.create({
    ...
    messages: [
      {
        role: "system",
        content: `You design live Vanguard sales practice scenarios from a selected real advisor meeting.
  ...
  try {
    ...
    const response = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: `You are a live sales coach observing a Vanguard practice conversation in progress.
  ...
  try {
    ...
    const response = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: `You are a live digital sales coach observing a Vanguard-style roleplay in progress.
  ...
  try {
    const stream = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: `You are a digital sales coach giving live post-call feedback on a Vanguard-style roleplay.
  ...
  try {
    ...
    const response = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: `You are a digital sales coach grading a Vanguard-style roleplay against the VG Way.
